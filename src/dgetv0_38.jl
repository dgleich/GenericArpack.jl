# Note that this uses the Arpack 3.8.0+ sequence that pushes the initial
# residual into the range of op.
# Previous versions do not do this and have a few bugs that result
# in either case. 
#=
c-----------------------------------------------------------------------
c\BeginDoc
...
function dgetv0!(
  ...
  if ido[] == 0
    ...
    elseif itry > 1 && BMAT == :G
      @warn "needs to be verified against arpack_jll"    
  ...
  while mainloop # the only way mainloop is false is if we are returning with ido[] = -1 above
    if first == false && orth == false # we are in the init state
      ...
      first = true
      if itry == 1
        # save op*x into resid
        @warn "needs to be verified against arpack_jll"
        copyto!(@view(resid[1:n]), @view(workd[(n+1):(2n)])) # dcopy (n, workd(n + 1), 1, resid, 1)
      end
      if BMAT == :G
    ...
    elseif first == true
      ...
      if BMAT == :G
        ...
        rnorm0 = norm(@view(resid[1:n]),2) # compute two-norm
      end
      rnorm[] = rnorm0
      if j==1
        # c     | Exit if this is the very first Arnoldi step |
    ...
    elseif orthstart == true
      ...
      if BMAT == :G
        ...
        rnorm[] = norm(@view(resid[1:n]),2) # compute two-norm
      end
