...
NP      Integer.  (INPUT/OUTPUT)
     ...
     The logi for adjusting is contained within the current
     subroutine.
...
H       Double precision (NEV+NP) by 2 array.  (OUTPUT)
     H is used to store the generated symmetri tridiagonal matrix
...
WORKD   Double precision work array of length 3*N.  (REVERSE COMMUNICATION)
     Distributed array to be used in the basi Lanczos iteration
...
function dsaup2!(
  ...
  @jl_arpack_check_length(ritz, maxcols)
  @jl_arpack_check_length(bounds, maxcols)
  @jl_arpack_check_length(workl, 3maxcols)
  @jl_arpack_check_length(workd, 3n)
  @jl_arpack_check_size(V, n, maxcols)
  @jl_arpack_check_size(H, maxcols, 2)
  @jl_arpack_check_size(Q, maxcols, maxcols)
  ...
  while true
    ...
    elseif startiter 
      ...
      iter += 1
      if msglvl > 0
        println(debug.logfile, "_saup2: **** Start of major iteration number **** ", iter)
      end
      if msglvl > 1
        println(debug.logfile, "_saup2: The length of the current Lanczos factorization ", nev[])
        println(debug.logfile, "_saup2: Extend the Lanczos factorization by ", np[])
      end
      ido[] = 0
      update = true 
    elseif update
      ...
      if msglvl > 1
        println(debug.logfile, "_saup2: Current B-norm of residual for factorization ", rnorm[])
      end 
      ...
      if ierr != 0 
        info = -8
        errorexit = true # go to 1200
        break
      end 
  
      # c        | Make a copy of eigenvalues and corresponding error |
      # c        | bounds obtained from _seigt.                       |
      copyto!(@view(workl[kplusp+1:2kplusp]), @view(ritz[1:kplusp]))
      copyto!(@view(workl[2kplusp+1:3kplusp]), @view(bounds[1:kplusp]))
      ...
      # c        | Convergence test. |
      copyto!(@view(workl[np0+1:np0+nev0]), @view(bounds[np0+1:np0+nev0]))
      nconv = dsconv(nev0, @view(ritz[np0+1:np0+nev0]), @view(workl[np0+1:np0+nev0]), tol)
      ...
      if (nconv >= nev0) || (iter > mxiter[]) || (np[] == 0)
        ...
        if which == :BE
          ...
          elseif which == :SA
            ...
            throw(ArgumentError("which $which choice is unknown in aupd2"))
          end 
          dsortr(wprime, true, kplusp, ritz, bounds)
        end
      ...
      elseif (nconv < nev[]) && (ishift == 1)
        ...
        =#
        nevbef = nev[]
        nev[] = nev[] + min(nconv, div(np[],2))
        if nev[] == 1 && kplusp >= 6
          nev[] = div(kplusp,2)
        elseif (nev[] == 1) && (kplusp > 2)
          nev[] = 2
        end
        np[] = kplusp - nev[]
        # c           | If the size of NEV was just increased |
      ...
      if msglvl > 0
        println(debug.logfile, "_saup2: no. of \"converged\" Ritz values at this iter: ", nconv)
        if msglvl > 1
          println(debug.logfile, "_saup2: NEV is: ", nev[])
          println(debug.logfile, "_saup2: NP is: ", np[])
          _arpack_vout(debug, "_saup2: \"wanted\" Ritz values", @view(ritz[np[]+1:np[]+nev[]]))
          _arpack_vout(debug, "_saup2: Ritz estimates of the  \"wanted\" values", 
            @view(bounds[np[]+1:np[]+nev[]]))
        end 
      end 
    ...
    elseif ushift
      ...
      # c        | in the exact shift case, dsgets already handles this.   |
      if ishift == 0 
        copyto!(@view(ritz[1:np[]]), @view(workl[1:np[]]))
      end 
      if msglvl > 2
      ...
      if BMAT == :G
        ...
        copyto!(@view(workd[n+1:2n]), @view(resid[1:n]))
        ipntr[1] = n+1
        ipntr[2] = 1
        ido[] = 2
        # c           | Exit in order to compute B*RESID |
        ...
        copyto!(@view(workd[1:n]), @view(resid[1:n]))
      end
    elseif cnorm
      # c        | Back from reverse communication; |
      # c        | WORKD(1:N) := B*RESID            |
      ...
      # at this point, we will do the mainiteration loop... 
      startiter = true 
    end
  end
