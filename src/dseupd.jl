...
function simple_dseupd!(
  ...
  if msglvl > 2
    _arpack_vout(debug, "_seupd: Ritz values passed in from _SAUPD.", 
      @view(workl[irz .+ ncv0]))
    _arpack_vout(debug, "_seupd: Ritz estimates passed in from _SAUPD.", 
      @view(workl[ibd .+ ncv0]))
  end
  ...
  if rvec
    ...
    np = ncv - nev
    ishift = 0 
    dsgets(ishift, which, nev, np, 
      @view(workl[irz:irz+ncv-1]), @view(workl[bounds:bounds+ncv-1]), workl; 
      debug, stats)
    
  
    if msglvl > 2
      _arpack_vout(debug, "_seupd: Ritz values after calling _SGETS.", 
        @view(workl[irz:irz+ncv-1]))
      _arpack_vout(debug, "_seupd: Ritz value indices after calling _SGETS.", 
        @view(workl[bounds:bounds+ncv-1]))
    end 
    ...
    =#
    if msglvl > 2
      println(debug.logfile, "_seupd: Number of specified eigenvalues ", numcnv)
      println(debug.logfile, "_seupd: Number of converged eigenvalues ", nconv)
    end
    ...
    if msglvl > 1
      ...
      # preserved for exact arpack equivlance. 
      # copy the last row of the eigenvector matrix away... 
      #copyto!(@view(workl[iw:iw+ncv-1]), @view(workl[iq+ncv-1:ldq:iq+ncv*ldq-1]))
      copyto!(@view(workl[iw:iw+ncv-1]), @view(Q[ncv,:]))
      _arpack_vout(debug, "_seupd: NCV Ritz values of the final H matrix",
        @view(workl[ihd:ihd+ncv-1]))
      _arpack_vout(debug, "_seupd: last row of the eigenvector matrix for H",
        @view(workl[iw:iw+ncv-1]))
    end 
    ...
    if reord 
      ...
      leftptr = 1
      rghtptr = ncv
      # if (ncv .eq. 1) go to 30 # THIS IS handled in the while statement
      while leftptr < rghtptr
        if Bool(select[leftptr])
          leftptr += 1
        elseif !Bool(select[rghtptr])
          rghtptr -= 1
        else
          #=
          c              | Swap the Ritz value on the left that has not |
          ...
          workl[ihd+leftptr-1] = workl[ihd+rghtptr-1]
          workl[ihd+rghtptr-1] = temp
          # seems like a simple swap would be easier, but kept like arpack!
          copyto!(@view(workl[iw.+ncv0]), @view(workl[iq+ncv*(leftptr-1) .+ ncv0]))
          copyto!(@view(workl[iq+ncv*(leftptr-1) .+ ncv0]), @view(workl[iq+ncv*(rghtptr-1) .+ ncv0]))
          copyto!(@view(workl[iq+ncv*(rghtptr-1) .+ ncv0]), @view(workl[iw.+ncv0]))
          # we can swap left, rghtptr...
          leftptr += 1
          rghtptr -= 1
        end
      end 
    ...
    if msglvl > 2
      _arpack_vout(debug, "_seupd: The eigenvalues of H--reordered", @view(workl[ihd.+ncv0]))
    end
    # c        | Load the converged Ritz values into D. |
    copyto!(@view(d[1:nconv]), @view(workl[ihd.+nconv0]))
  else
    # c        | Ritz vectors not required. Load Ritz values into D. |
    copyto!(@view(d[1:nconv]), @view(workl[ritz.+nconv0]))
    copyto!(@view(workl[ihd .+ ncv0]), @view(workl[ritz.+ncv0]))
  end
  ...
  if type == :REGULR
    ...
    if rvec
      ...
      # call dsesrt ('LA', rvec , nconv, d, ncv, workl(iq), ldq)
      # we sort the top nconv vectors... note that ldq = ncv 
    ...
    c        |  They are only reordered.                                   |
    =#
    copyto!(@view(workl[iw .+ ncv0]), @view(workl[ihd .+ ncv0]))
    if type == :SHIFTI
      for k=1:ncv
        workl[ihd+k-1] = one(T)/workl[ihd+k-1] + sigma 
      end 
    elseif type == :BUCKLE
      for k=1:ncv
        workl[ihd+k-1] = sigma*workl[ihd+k-1]/(workl[ihd+k-1] - one(T))
      end 
    elseif type == :CAYLEY
      for k=1:ncv
        workl[ihd+k-1] = sigma*(workl[ihd+k-1]+one(T))/(workl[ihd+k-1] - one(T))
      end 
    end 
    ...
    c        |  Ritz vector purification.                                  |
    =#
    copyto!(@view(d[1 .+ nconv0]), @view(workl[ihd .+ nconv0]))
    dsortr(:LA, true, nconv, @view(workl[ihd .+ nconv0]), @view(workl[iw .+ nconv0]))
    if rvec
      # same as the call above... 
      dsortr(:LA, true, nconv, @view(d[1 .+ nconv0]), 
  ...
  c     | the Lanczos basis matrix V.                    |
  =#
  if rvec # && howmny == :A, which isn't used...
    #=
    c        | Compute the QR factorization of the matrix representing  |
    c        | the wanted invariant subspace located in the first NCONV |
    c        | columns of workl(iq,ldq).                                |
    ...
    workl[ihb+ncv-1] = one(T)
    # in Julia, we need an extra byref variable, for work
    # Arpack/Fortran just uses the temp variable, Julia
    # semantics mean we can't take the ref of that... 
  ...
  elseif type != :REGULR && rvec
    ...
    c        | *  Determine Ritz estimates of the lambda.      |
    =#
    _dscal!(bnorm2, @view(workl[ihb .+ ncv0]))
    # TODO, double check these, 
    if type == :SHIFTI
      for k=1:ncv
        workl[ihb+k-1] = abs(workl[ihb+k-1])/(workl[iw+k-1]*workl[iw+k-1])
      end
    elseif type == :BUCKLE
      for k=1:ncv
        workl[ihb+k-1] = sigma*abs(workl[ihb+k-1])/((workl[iw+k-1]-one(T))^2)
      end
    elseif type == :CAYLEY
      for k=1:ncv
        # DOUBLE CHECK the order of operations here...
        workl[ihb+k-1] = abs(workl[ihb+k-1]/workl[iw+k-1]*(workl[iw+k-1]-one(T)) )
      end
    end
  end 

  if type != :REGULR && msglvl > 1
    _arpack_vout(debug, "_seupd: Untransformed converged Ritz values", 
      @view(d[1:nconv]))
    _arpack_vout(debug, "_seupd: Ritz estimates of the untransformed Ritz values", 
      @view(workl[ihb .+ nconv0]))
  elseif msglvl > 1
    _arpack_vout(debug, "_seupd: Converged Ritz values", 
      @view(d[1:nconv]))
    _arpack_vout(debug, "_seupd: Associated Ritz estimates", 
      @view(workl[ihb .+ nconv0]))
  end 
  ...
  elseif rvec && type == :BUCKLE
    for k=0:nconv-1
      workl[iw+k] = workl[iw+ncv+k]/(workl[iw+k]-one(T))
    end 
  end

  if rvec && type != :REGULR
    # rank 1 update
    _dger!(one(T), @view(resid[1:n]), @view(workl[iw .+ nconv0]), @view(Z[1:n, 1:nconv]))
  end 
